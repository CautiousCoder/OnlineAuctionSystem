...
<div class="tagindex p-2">
  ...
  <div class="card">
    ...
    {{-- <div class="card-body p-0"> --}}
      ...
      <form action="{{ route('seller.tag.update', [$tag->id]) }}" method="POST">
        @method('PUT')
        @csrf
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <textarea class="form-control" name="des" id="desc"  rows="4" placeholder="Descriptions">{{ $tag->description }}</textarea>
          </div>
        </div>
        <!-- /.card-body -->

        <div class="card-footer mb-2 d-flex justify-content-between">
          <button type="submit" class="btn btn-lg btn-primary m-auto">Update Tag</button>
        </div>
      </form>
    {{-- </div> --}}
      <!-- /.form -->
